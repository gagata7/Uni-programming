...
(define-type Exp
  (try [e1 : Exp] [e2 : Exp]) ;dodalam nowy typ wyrazenia {try e1 e2}
  (numE [n : Number])
...
(define (parse-exp [s : S-Exp]) : Exp
  (cond
    [(s-exp-match? `{try ANY ANY} s)
     ;jesli moje s-wyrazenie jest postaci {try e1 e2} to zapisz je za pomocą skladni abst.
     (try (parse-exp (second (s-exp->list s)))
...
(define (eval [e : Exp]) : Answer
  (type-case Exp e
    [(try e1 e2) ;jesli moje wyrazenie jest typu {try e1 e2} to:
     (let ([result (eval e1)]) ;result to policzonego wyrazenia e1
       ;result moze byc albo Value albo Errorem
       (type-case Answer result 
         [(valueA v) (return v)] ;jesli (eval e1) jest liczba, to ja zwroc
         [(errorA loc msg) (eval e2)]))] ;jesli jest bledem - to oblicz e2 i zwroc
    [(numE n)
     (return (numV n))]
    [(opE o l r)
     (do ([v-l (eval l)]
...
(module+ test
  ...
  ;-----------------------------------------
  (test (run `{try (error blad "to jest blad") 4}) (valueA (numV 4)))
  (test (run `{try (* 6 6) 6}) (valueA (numV 36)))
)
