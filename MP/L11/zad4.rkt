  ...
  (test (run `{let x 1
        ...
        (runA `{let x 1
                 {+ x {let x {+ x 1}
                        {* x 3}}}}))
 ;runTB-------------------------------------------------------------------
  (test (runTB `{let x 1 {+ x {let x {+ x 1} {* x 3}}}}) (numV 7))
  (test (runTB `{let x 3 {+ x {let y 5 {* x y}}}}) (numV 18))
)
...
(define (translate_back [e : ExpA] [env : Env]) : Exp
  (type-case ExpA e
    ...
    [(varA n) ;indeks de Bruijna
     (varE (fresh-name (- (length env) n)))]
     ;mowi ile letow temu pojawilo sie zwiazanie tej zmiennej
     ;(length env) to to w ilu letach program juz sie zagniezdzil
     ;(length env)-n mowi ile letow od poczatku znajduje sie zmienna
    [(letA a1 a2)
     (letE (fresh-name (+ 1 (length env)));1. zrob nowy symbol x_i
           (translate_back a1 env) ;2. przypisz mu a1
           (translate_back a2 (extend-env env
             (fresh-name (+ 1 (length env))) (eval (translate_back a1 env) env))))
           ;3. wykonaj a2 z rozszerzonym srodowiskiem o x_i = (obliczone a1)
    ]
  )
)
...
;funkcja pomocnicza - sprawdzajaca czy translate-back dziala
