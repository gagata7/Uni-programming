...
(define (merge s1 s2)
  (let ([x (stream-car s1)] [y (stream-car s2)])
    (cond
      [(= x y) (stream-cons (stream-car s1) (merge (stream-cdr s1) (stream-cdr s2)))]
      [(< x y) (stream-cons (stream-car s1) (merge (stream-cdr s1) s2))]
      [else (stream-cons (stream-car s2) (merge s1 (stream-cdr s2)))])))
...
(stream-ref hamming-stream 3)
(stream-ref hamming-stream 4)
(stream-ref hamming-stream 5)
(stream-ref hamming-stream 6)