...
(define-type Exp
  ...
  (setE [x : Symbol] [e : Exp])
  (whileE [b : Exp] [e : Exp])) ;dopóki b spełniony, wykonuj e
...
(module+ test
  ...
  (test (parse `{double 9})
   ...
   (test (run `{let x 1 {begin {while {<= x 5} {set! x {+ x 1}}} x}})
        (numV 6)))
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    [(whileE b e0)
     (let ([war (eval b env)]) ;war = zewaluowany warunek pętli
       (type-case Value war
         [(boolV bb) ;jeśli warunek jest boolem - czyli tak jak chcemy, żeby był
          (if bb (begin ;jeśli warunek jest prawdziwy, to wykonaj e0
                   (eval e0 env)
                   (eval e env)) ;a potem ponownie zrób e czyli tego while
              ;i rób to tak długo, dopóki warunek w while będzie spełniony
              (voidV))] ;jeśli while nie będzie spełniony to zakończ pętle
         [else (error 'eval "while: logical test did not evaluate to bool")]))]
    ;jeśli warunek pętli nie jest boolem to zwróć błąd
    [(numE n) (numV n)]
    [(ifE b l r)
