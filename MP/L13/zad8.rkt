...
(define-type Exp
  ...
  (setE [x : Symbol] [e : Exp])
  (readE) ;to tylko odczytuje wiec nie ma parametrow
  (writeE [e : Exp])) ;to ma jakies wyrazenie
  ...
  (type-case Exp e
    ...
    [(setE x e0)
     (begin 
       ...
       (voidV))]
    [(readE) (eval (parse (read)) env)]; to najpierw readem wczyta na wejscie
    ;potem od razu sparsuje wczytane s-wyrazenie
    ;a nastepnie zewaluuje sparsowane z podanym srodowiskiem
    [(writeE e)
     (begin
       (print-value (eval e env)) ;zwyczajnie wypisz wyrazenie e
       (display "\n")
       (voidV) ;musi byc void, bo eval zawsze musi cos zwracac
       ;w przypadku wypisywania, tylko sie wypisuje cos ale nic nie zwraca
       ;dlatego (writeE e) ewaluuje się do (void)
     )
    ]))
